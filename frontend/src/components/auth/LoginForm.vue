    ...
    <form @submit.prevent="handleSubmit" class="space-y-6">
        ...
        <div class="text-center mb-6">
            ...
            <p class="text-sm text-secondary-400">
                Accédez à votre table virtuelle
            </p>
        </div>
        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center">
                <input
                    ...
                    type="checkbox"
                    :disabled="isLoading"
                    class="h-4 w-4 bg-secondary-700 border-secondary-600 rounded
...
<script setup lang="ts">
    ...
    const isFormValid = computed(() => {
        return form.value.email.length > 0 && form.value.password.length > 0 && isValidEmail(form.value.email)
    })
    ...
    const validateForm = (): boolean => {
        ...
        if (!form.value.password) {
            ...
            validationErrors.value.push({ field: 'password', message: 'Le mot de passe doit faire au moins 3 caractères' })
        }
