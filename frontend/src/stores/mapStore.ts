...
import type { GameMap, GameToken, CreateTokenDTO, MoveTokenDTO } from '@/types/game';
...
export const useMapStore = defineStore('map', () => {
  ...
  async function loadActiveMap(gameId: number) {
    ...
    try {
      ...
      if (e && typeof e === 'object' && 'message' in e) {
        error.value = (e as { message: string }).message || 'Erreur lors du chargement de la carte active';
      } else {
  ...
  async function activateMap(gameId: number, mapId: number) {
    ...
    try {
      ...
      if (e && typeof e === 'object' && 'message' in e) {
        error.value = (e as { message: string }).message || "Erreur lors de l'activation de la carte";
      } else {
   ...
   */
  function buildCreateTokenDTO(data: Partial<CreateTokenDTO> & { name: string; type: TokenType; x: number; y: number }): CreateTokenDTO {
   ...
   * Déplacer un token
   */
  async function moveToken(tokenId: number, x: number, y: number, rotation?: number): Promise<GameToken> {
    ...
    try {
      ...
      const updatedToken = await tokenApi.move(currentGameId.value, activeMap.value.id, tokenId, moveData);
   ...
   */
  async function updateToken(tokenId: number, updates: any) {
    if (!currentGameId.value || !activeMap.value) {
      throw new Error('GameId or Map not set');
    }
    ...
    try {
      const updatedToken = await tokenApi.partialUpdate(currentGameId.value, activeMap.value.id, tokenId, updates);
      updateTokenInList(updatedToken);
      return updatedToken;
    } catch (e: unknown) {
      if (e && typeof e === 'object' && 'message' in e) {
  ...
  async function toggleTokenVisibility(tokenId: number) {
    ...
    try {
      ...
      if (e && typeof e === 'object' && 'message' in e) {
        error.value = (e as { message: string }).message || 'Erreur lors du changement de visibilité';
      } else {
  ...
  async function toggleTokenLock(tokenId: number) {
    ...
    try {
      ...
      if (e && typeof e === 'object' && 'message' in e) {
        error.value = (e as { message: string }).message || 'Erreur lors du changement de verrouillage';
      } else {
   ...
   */
  function handleTokenEvent(data: any) {
   ...
   */
  function handleMapEvent(data: any) {
