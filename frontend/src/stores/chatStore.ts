...
export const useChatStore = defineStore('chat', () => {
  ...
  const chatMessages = computed(() => {
    return messages.value.filter(msg => 
      msg.type === 'chat' || msg.type === 'emote' || msg.type === 'whisper'
    );
  });
  ...
  async function loadMessagesSince(gameId: number, since: string) {
    try {
      ...
      if (e && typeof e === 'object' && 'message' in e) {
        error.value = (e as { message: string }).message || 'Erreur lors du chargement des nouveaux messages';
      } else {
  ...
  async function sendSystemMessage(gameId: number, content: string) {
    ...
    try {
      ...
      if (e && typeof e === 'object' && 'message' in e) {
        error.value = (e as { message: string }).message || 'Erreur lors de l\'envoi du message système';
      } else {
  ...
  async function deleteMessage(messageId: number) {
    ...
    try {
      ...
      if (e && typeof e === 'object' && 'message' in e) {
        error.value = (e as { message: string }).message || 'Erreur lors de la suppression du message';
      } else {
  ...
  function handleChatMessage(data: any) {
    ...
    if (data.messageId) {
      const message: GameMessage = {
        ...
        diceTotal: data.diceResult?.total || null,
   ...
   */
  function handleDiceRoll(data: any) {
